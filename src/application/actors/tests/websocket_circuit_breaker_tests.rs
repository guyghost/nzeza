...
use crate::application::actors::{
    CircuitBreakerConfig, CircuitBreakerEvent, CircuitEventType, CircuitState, ConnectionState,
    ExponentialBackoffConfig, WebSocketClient,
};
...
async fn test_circuit_opens_after_threshold() {
    ...
    let client = WebSocketClient::new(&format!("ws://{}", server_addr)).with_circuit_breaker(
    ...
    assert!(
        blocked_attempt.is_err(),
        "Connection attempts should be blocked when circuit is open"
    );

    let error_message = blocked_attempt.unwrap_err();
    assert!(
        error_message.contains("circuit") || error_message.contains("open"),
    ...
    assert!(
        failure_history
            .iter()
            .all(|f| f.is_within_window(Duration::from_secs(60))),
...
async fn test_circuit_half_open_after_timeout() {
    ...
    let open_event = timeout(Duration::from_secs(5), async {
        while let Ok(event) = circuit_receiver.recv().await {
            if let CircuitBreakerEvent::StateChanged {
    ...
    assert!(
        half_open_event.unwrap_or(false),
    ...
    assert!(
        client.is_circuit_half_open().await,
    ...
    info!(
        "Half-open connection attempt result: {:?}",
        half_open_attempt
    );
...
async fn test_circuit_closes_on_success() {
    ...
    for attempt in 1..=5 {
        ...
        while let Ok(event) = timeout(Duration::from_millis(50), circuit_receiver.recv()).await {
            if let Ok(CircuitBreakerEvent::StateChanged {
    ...
    assert!(
        circuit_closed,
        "Circuit should close after successful connections"
    );
    assert_eq!(client.circuit_state(), CircuitState::Closed);
    assert!(
        success_count >= 3,
        "Should require at least 3 successes to close"
    );
    ...
    assert!(
        !client.is_circuit_half_open().await,
        "Should not be in half-open state"
    );
    ...
    assert!(
        ...
        "Normal connections should work when circuit is closed"
    );
    ...
    assert!(
        success_history
            .iter()
            .all(|s| s.occurred_recently(Duration::from_secs(10))),
    ...
    assert_eq!(
        reset_metrics.consecutive_failures, 0,
        "Failures should be reset after closing"
    );
...
async fn test_exponential_backoff_during_open() {
    ...
    let timeout_durations = backoff_monitor_task
        .await
        .expect("Backoff monitor should complete");

    // Verify exponential backoff pattern
    assert!(
        timeout_durations.len() >= 3,
        "Should have multiple timeout attempts"
    );
    ...
    let max_timeout_reached = timeout_durations
        .iter()
    ...
    if timeout_durations.len() > 6 {
        assert!(
            max_timeout_reached,
            "Should reach maximum timeout for long sequences"
        );
    }
...
async fn test_circuit_metrics_collection() {
    ...
    assert!(
        metrics_json.contains("current_state"),
        "Should include state info"
    );
    assert!(
        metrics_json.contains("total_failures"),
        "Should include failure count"
    );
    assert!(
        metrics_json.contains("total_successes"),
        "Should include success count"
    );

    let metrics_prometheus = client.export_metrics_as_prometheus().await;
    assert!(
        !metrics_prometheus.is_empty(),
        "Should export metrics in Prometheus format"
    );
    assert!(
        metrics_prometheus.contains("circuit_breaker_state"),
        "Should include Prometheus metrics"
    );
    ...
    assert_eq!(
        reset_metrics.total_successes, 0,
        "Successes should be reset"
    );
    assert_eq!(
        reset_metrics.state_transitions, 0,
        "Transitions should be reset"
    );
    assert!(
        reset_metrics.metrics_reset_time.unwrap() >= reset_time,
        "Should record reset time"
    );
