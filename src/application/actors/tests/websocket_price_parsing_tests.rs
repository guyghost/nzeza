...
use crate::application::actors::{
    ConnectionState, ParsingError, ParsingErrorType, ParsingMetrics, PrecisionMetrics, PriceUpdate,
    TypeError, TypeValidationMetrics, ValidationError, ValidationMetrics, WebSocketClient,
};
...
async fn test_valid_price_message_parsing() {
    ...
    for price in &parsed_prices {
        ...
        assert!(
            timestamp_since_epoch <= duration_since_epoch,
...
async fn test_malformed_json_handling() {
    ...
    client
        .queue_outbound_message(&valid_message.to_string())
        .await;
    ...
    assert!(
        parsing_errors.len() >= 6,
        "Should capture most malformed messages as errors"
    );

    // Verify error types
    let json_syntax_errors = parsing_errors
        .iter()
        .filter(|e| matches!(e.error_type, ParsingErrorType::JsonSyntaxError))
        .count();
    assert!(
        json_syntax_errors >= 4,
        "Should have multiple JSON syntax errors"
    );

    let invalid_structure_errors = parsing_errors
        .iter()
        .filter(|e| matches!(e.error_type, ParsingErrorType::InvalidStructure))
        .count();
    assert!(
        invalid_structure_errors >= 1,
    ...
    assert!(
        client.is_connected(),
        "Connection should remain stable after errors"
    );
    ...
    let parsing_metrics = client.parsing_metrics();
    assert!(
        parsing_metrics.parsing_errors >= 6,
        "Should count parsing errors"
    );
    assert_eq!(
        parsing_metrics.successful_parses, 1,
        "Should count successful parses"
    );
    assert!(
        parsing_metrics.total_messages_parsed >= 7,
        "Should count total messages"
    );
    assert!(
        parsing_metrics.error_recovery_count >= 6,
        "Should count error recoveries"
    );
...
async fn test_missing_required_fields() {
    ...
    client
        .queue_outbound_message(&complete_message.to_string())
        .await;
    ...
    let type_error = validation_errors
        .iter()
    ...
    assert!(
        valid_price_result.is_ok(),
        "Should accept complete valid messages"
    );
    ...
    assert_eq!(
        validation_metrics.total_validations, 8,
        "Should validate all messages"
    );
    assert_eq!(
        validation_metrics.validation_failures, 7,
        "Should count failures"
    );
    assert_eq!(
        validation_metrics.validation_successes, 1,
        "Should count successes"
    );
    assert!(
        validation_metrics.missing_field_errors >= 6,
        "Should count missing field errors"
    );
    assert!(
        validation_metrics.type_mismatch_errors >= 1,
        "Should count type errors"
    );
...
async fn test_price_type_validation() {
    ...
    assert_eq!(
        valid_prices.len(),
        3,
        "Should accept exactly 3 valid prices"
    );
    assert!(
        type_errors.len() >= 6,
        "Should reject at least 6 invalid prices"
    );

    // Verify valid prices
    let btc_price = valid_prices
        .iter()
        .find(|p| p.product_id == "BTC-USD")
        .unwrap();
    assert_eq!(btc_price.price, 45000.50);

    let eth_price = valid_prices
        .iter()
        .find(|p| p.product_id == "ETH-USD")
        .unwrap();
    assert_eq!(eth_price.price, 3200.75);

    let shib_price = valid_prices
        .iter()
        .find(|p| p.product_id == "SHIB-USD")
        .unwrap();
    assert!((shib_price.price - 0.0000123).abs() < 1e-10);

    // Verify type error details
    let non_numeric_error = type_errors.iter().find(|e| {
        e.field_name == "price" && e.actual_type == "string" && e.reason.contains("non-numeric")
    });
    assert!(
        non_numeric_error.is_some(),
        "Should detect non-numeric string"
    );

    let boolean_error = type_errors
        .iter()
        .find(|e| e.field_name == "price" && e.actual_type == "boolean");
    assert!(boolean_error.is_some(), "Should detect boolean type");

    let negative_error = type_errors
        .iter()
    ...
    assert_eq!(
        type_metrics.total_type_checks, 10,
        "Should check all message types"
    );
    assert_eq!(
        type_metrics.type_validation_successes, 3,
        "Should count successes"
    );
    assert!(
        type_metrics.type_validation_failures >= 6,
        "Should count failures"
    );
    assert!(
        type_metrics.non_numeric_price_errors >= 3,
        "Should count non-numeric errors"
    );
    assert!(
        type_metrics.negative_price_errors >= 1,
        "Should count negative price errors"
    );
    assert!(
        type_metrics.zero_price_errors >= 1,
        "Should count zero price errors"
    );
...
async fn test_decimal_precision_preservation() {
    ...
    for (i, (expected_product_id, original_price_str, expected_price)) in
        precision_tests.iter().enumerate()
    {
        ...
        let tolerance = if *expected_price < 1e-10 {
            1e-20
        } else {
            1e-15
        };
        ...
        if let Some(original_str) = &parsed_price.original_price_string {
            assert_eq!(
                ...
                "Should preserve original price string for {}",
                expected_product_id
            );
        }
        ...
        assert!(
            parsed_price.decimal_places >= decimal_places as u8,
    ...
    let btc_price = parsed_prices
        .iter()
        .find(|p| p.product_id == "BTC-USD")
        .unwrap();
    let eth_price = parsed_prices
        .iter()
        .find(|p| p.product_id == "ETH-USD")
        .unwrap();
