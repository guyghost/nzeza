...
use crate::application::actors::{
    ConnectionErrorMetrics, ConnectionPreventionMetrics, ConnectionState, DisconnectEvent,
    DisconnectMetrics, DisconnectType, FrameBufferMetrics, LargeMessageMetrics,
    MessageOrderingMetrics, MixedMessageMetrics, ProgressEvent, StateChangeEvent,
    StateTransitionMetrics, TimeoutMetrics, WebSocketClient,
};
...
async fn test_multiple_concurrent_connections() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        let (result1, result2, result3) =
            tokio::join!(client1.connect(), client2.connect(), client3.connect());
...
async fn test_concurrent_message_reading() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        let send_task = tokio::spawn(async move {
            let _ = sender.send(
                r#"{"product_id":"BTC-USD","price":"45000.50","timestamp":"2025-10-28T12:00:00Z"}"#
                    .to_string(),
            );
            sleep(Duration::from_millis(50)).await;
            let _ = sender.send(
                r#"{"product_id":"ETH-USD","price":"3200.75","timestamp":"2025-10-28T12:00:01Z"}"#
                    .to_string(),
            );
            sleep(Duration::from_millis(50)).await;
            let _ = sender.send(
                r#"{"product_id":"SOL-USD","price":"95.25","timestamp":"2025-10-28T12:00:02Z"}"#
                    .to_string(),
            );
        });
        ...
        assert!(
            messages[0].contains("BTC-USD"),
            "First message should be BTC-USD"
        );
        assert!(
            messages[1].contains("ETH-USD"),
            "Second message should be ETH-USD"
        );
        assert!(
            messages[2].contains("SOL-USD"),
            "Third message should be SOL-USD"
        );
        assert!(
            messages[0].contains("45000.50"),
            "First message should contain price"
        );
        assert!(
            messages[1].contains("3200.75"),
            "Second message should contain price"
        );
        assert!(
            messages[2].contains("95.25"),
            "Third message should contain price"
        );
...
async fn test_websocket_auth_validation() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        assert_eq!(
            client_no_auth.connection_state(),
        ...
        assert!(
            bad_auth_result.is_err(),
            "Connection with bad auth should fail"
        );
        assert_eq!(
            client_bad_auth.connection_state(),
            ConnectionState::Disconnected
        );

        // Test connection with valid auth (should succeed)
        let valid_token = "valid_bearer_token_abcdef123456";
        let client_good_auth =
            WebSocketClient::new(&format!("ws://{}", server_addr)).with_bearer_token(valid_token);
        let good_auth_result = client_good_auth.connect().await;
        assert!(
            good_auth_result.is_ok(),
            "Connection with valid auth should succeed"
        );
        assert_eq!(
            client_good_auth.connection_state(),
        ...
        assert!(
            auth_header.contains(valid_token),
        ...
        assert_eq!(
            client_good_auth.current_token(),
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        let send_invalid_task = tokio::spawn(async move {
            ...
            let _ = message_sender.send(
                r#"{"product_id":"BTC-USD","price":"45000.00","timestamp":"2025-10-28T12:00:00Z"}"#
                    .to_string(),
            );
        });
        ...
        while let Ok(error_result) =
            timeout(Duration::from_millis(500), error_receiver.recv()).await
        {
        ...
        assert!(
            error_metrics.malformed_json_count > 0,
            "Should count malformed JSON errors"
        );
        assert!(
            error_metrics.invalid_frame_count > 0,
            "Should count invalid frame errors"
        );
        assert!(
            error_metrics.total_error_count >= 3,
            "Should count total errors"
        );
        assert!(
            error_metrics.last_error_timestamp.is_some(),
            "Should have last error timestamp"
        );
...
async fn test_connection_timeout_handling() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        assert!(
            ...
            "Error should mention timeout/connection/refused: {}",
            error_message
        );
        ...
        assert!(
            timeout_metrics.connection_timeouts <= 1,
            "Should have 0 or 1 timeout"
        );
...
async fn test_graceful_disconnect() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        assert_eq!(client.connection_state(), ConnectionState::Disconnected);
        assert!(
            disconnect_duration <= Duration::from_secs(3),
            "Disconnect should complete quickly"
        );

        // Verify disconnect event was sent
        let disconnect_event_result =
            timeout(Duration::from_millis(500), disconnect_receiver.recv()).await;
        assert!(
            disconnect_event_result.is_ok(),
...
async fn test_forced_disconnect() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        assert_eq!(client.connection_state(), ConnectionState::Disconnected);
        assert!(
            disconnect_duration <= Duration::from_millis(200),
            "Force disconnect should be immediate"
        );

        // Verify disconnect event
        let disconnect_event_result =
            timeout(Duration::from_millis(300), disconnect_receiver.recv()).await;
        assert!(
            disconnect_event_result.is_ok(),
            "Should receive disconnect event"
        );
        ...
        assert!(
            !disconnect_event.clean_shutdown,
            "Should not be clean shutdown"
        );
...
async fn test_connection_state_transitions() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        let connect_task = tokio::spawn(async move { client_clone.connect().await });
        ...
        while monitor_start.elapsed() < Duration::from_secs(5) && transitions.len() < 3 {
            if let Ok(state_change_result) =
                timeout(Duration::from_millis(100), state_receiver.recv()).await
            {
        ...
        assert!(
            transitions.len() >= 2,
            "Should have at least Disconnected->Connecting->Connected"
        );
        ...
        for transition in &transitions {
            assert!(
                transition.timestamp.is_some(),
                "Each transition should have timestamp"
            );
            assert!(
                transition.duration > Duration::ZERO,
                "Each transition should have duration"
            );
            assert!(
                !transition.trigger_reason.is_empty(),
                "Each transition should have reason"
            );
        }
        ...
        let disconnect_transition_result =
            timeout(Duration::from_secs(1), state_receiver.recv()).await;
        assert!(
            disconnect_transition_result.is_ok(),
            "Should receive disconnect transition"
        );
        ...
        assert_eq!(
            disconnect_transition.to_state,
            ConnectionState::Disconnected
        );
        ...
        assert_eq!(
            transition_metrics.current_state,
            ConnectionState::Disconnected
        );
...
async fn test_double_connection_prevention() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        assert!(
            client.is_connected(),
            "Should be connected after first connection"
        );
        ...
        assert!(
            error_message.contains("already") || error_message.contains("connected"),
        ...
        assert!(
            !original_connection_id.is_empty(),
            "Should have connection ID"
        );
        ...
        assert!(
            ...
            "Third connection should also fail"
        );
        ...
        let (concurrent_result1, concurrent_result2) =
            tokio::join!(client_clone1.connect(), client_clone2.connect());

        // Both should fail
        assert!(
            concurrent_result1.is_err(),
            "Concurrent connection 1 should fail"
        );
        assert!(
            concurrent_result2.is_err(),
            "Concurrent connection 2 should fail"
        );
...
async fn test_connection_failure_handling() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        assert!(
            rejection_error.contains("reject") || rejection_error.contains("refused"),
            "Error should mention rejection: {}",
            rejection_error
        );

        // Test case 2: Invalid protocol
        let client_invalid_protocol =
            WebSocketClient::new("invalid://127.0.0.1:9999").with_retry_on_failure(false);
        ...
        assert!(
            protocol_error.contains("protocol") || protocol_error.contains("scheme"),
        ...
        assert!(
            dns_error.contains("resolve") || dns_error.contains("name"),
        ...
        assert!(
            ...
            "Unreachable address should fail"
        );
        ...
        assert!(
            ...
            "Handshake failure should fail connection"
        );
...
async fn test_frame_buffering() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        mock_server
            .send_raw_message(fragment1)
            .await
            .expect("Failed to send fragment 1");
        sleep(Duration::from_millis(50)).await;
        mock_server
            .send_raw_message(fragment2)
            .await
            .expect("Failed to send fragment 2");
        sleep(Duration::from_millis(50)).await;
        mock_server
            .send_raw_message(fragment3)
            .await
        ...
        assert!(
            reassembled_result.is_ok(),
            "Should receive reassembled message"
        );

        let reassembled_message = reassembled_result.unwrap().unwrap();
        assert_eq!(
            reassembled_message, large_message,
            "Message should be properly reassembled"
        );
        ...
        mock_server
            .send_raw_message(&oversized_message)
            .await
        ...
        if oversized_message.len() > 1024 {
            assert!(
                buffer_metrics.buffer_overflows >= 1,
        ...
        while concurrent_count < 10 && Instant::now() < concurrent_deadline {
        ...
        assert!(
            concurrent_count >= 8,
            "Should receive most concurrent messages"
        );
...
async fn test_mixed_valid_invalid_messages() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        assert_eq!(
            valid_messages.len(),
            expected_valid,
            "Should receive exactly {} valid messages",
            expected_valid
        );
        assert!(
            error_messages.len() >= expected_invalid - 1, // Allow for some error tolerance
            "Should receive at least {} errors",
            expected_invalid - 1
        );

        // Verify valid message content
        assert!(
            valid_messages.iter().any(|m| m.contains("message1")),
            "Should contain message1"
        );
        assert!(
            valid_messages.iter().any(|m| m.contains("message2")),
            "Should contain message2"
        );
        assert!(
            valid_messages.iter().any(|m| m.contains("message3")),
            "Should contain message3"
        );
        assert!(
            valid_messages.iter().any(|m| m.contains("message4")),
            "Should contain message4"
        );
        assert!(
            valid_messages.iter().any(|m| m.contains("message5")),
            "Should contain message5"
        );
        ...
        for valid_msg in &valid_messages {
            ...
            assert!(
                ...
                "Valid message should parse as JSON: {}",
                valid_msg
            );
        }
...
async fn test_large_message_handling() {
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        mock_server
            .send_raw_message(&message_1mb_json)
            .await
            .expect("Failed to send 1MB message");
        ...
        assert!(
            progress_updates.last().unwrap().completed,
            "Should complete"
        );
        ...
        mock_server
            .send_raw_message(&message_5mb_json)
            .await
            .expect("Failed to send 5MB message");

        let large_message_result = timeout(Duration::from_secs(30), message_receiver.recv()).await;
        assert!(
            large_message_result.is_ok(),
            "Should receive 5MB message within timeout"
        );
        ...
        assert!(
            large_message.contains("5MB"),
            "Should contain size indicator"
        );
        assert!(
            large_message.len() >= 5 * 1024 * 1024,
        ...
        mock_server
            .send_raw_message(&oversized_json)
            .await
            .expect("Failed to send oversized message");
        ...
        let concurrent_task = tokio::spawn(async move {
            for i in 0..concurrent_count {
                ...
                let concurrent_json = format!(
                    r#"{{"data": "{}", "id": {}, "size": "512KB"}}"#,
                    concurrent_msg, i
                );
                mock_server_clone
                    .send_raw_message(&concurrent_json)
                    .await
                    .expect("Failed to send concurrent message");
                sleep(Duration::from_millis(100)).await;
            }
        ...
        concurrent_task
            ...
            .expect("Concurrent task should complete");
        assert!(
            concurrent_received >= concurrent_count - 1,
